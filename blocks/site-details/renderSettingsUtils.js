...
export async function renderUpdatesSection(div, { projectDetails, authHeaders }) {
  div.innerHTML = '';
  const endpoint = `${SCRIPT_API}/${projectDetails.darkAlleyProject ? 'daUpdateProject' : 'updateProject'}/`;
  const versionInfo = await fetch(`${endpoint}checkUpdates/${projectDetails.projectSlug}`, { headers: authHeaders }).then((res) => res.json()).catch(() => null);
