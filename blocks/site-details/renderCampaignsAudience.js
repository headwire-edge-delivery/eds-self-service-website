...
export default async function renderCampaignsAudience({ container, nav, renderOptions }) {
  ...
  const editButton = parseFragment(`
    <a href="${projectDetails.darkAlleyProject ? `https://da.live/sheet#/da-self-service/${siteSlug}/recipients` : `https://docs.google.com/spreadsheets/d/${audienceSheetData.id}/edit`}" class="button primary action" target="_blank">Edit</a>
  `);
